	...
	def train(self, mb_states, mb_actions, mb_old_values, mb_advs, mb_returns, mb_old_a_logps):
		...
		for i in range(self.sample_n_epoch):
			...
			for j in range(self.sample_n_mb):
				...
				pg_loss_unclipped = ratio * sample_advs

				# Compute the clipped objective
				pg_loss_clipped = torch.clamp(ratio, 1.0 - self.clip_val, 1.0 + self.clip_val) * sample_advs
				
				# Final policy gradient loss is the negative mean of the minimum of the two
